...
import {Context, inject, BindingScope, Constructor} from '@loopback/context';
...
describe('Routing', () => {
  ...
  describe('current controller', () => {
    ...
    it('binds current controller resolved from a singleton binding', async () => {
      givenControllerAsSingletonInApp(app, controllerClass);
  ...
  function givenControllerAsSingletonInApp(
    app: Application,
    controller: ControllerClass<ControllerInstance>,
  ) {
    return app.controller(controller).inScope(BindingScope.SINGLETON);
  }
